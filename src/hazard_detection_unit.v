module hazard_detection_unit (
    ...
    wire lw_use;
    assign lw_use = ((ID_EX_MemRead) && ((ID_EX_RegisterRd[4:0] == IF_ID_Rs1[4:0]) || (ID_EX_RegisterRd[4:0] == IF_ID_Rs2[4:0]) ) //lw-use
        & (~is_lui) & (~is_auipc) &(~is_jal)); //stuff)
	...
	always @(*) begin
		...
		if (boot_cnt <= 2'b01)
			stall_cnt = 1'b0 ;
		else if ((~we_were_wrong_EX_MEM) && lw_use)
			stall_cnt = 1'b1 ;
		else
			stall_cnt = 1'b0 ;
